...
class BancoINVICOSdoFinalService(
    ...
    def __post_init__(self):
        ...
        super().__init__(
            ...
            filter_schema=BancoINVICOSdoFinalFullFilter,  # <--- LE DECIMOS QUIÉN ES 'F'
        )

    # -------------------------------------------------
    async def add_many(
        self, data: List[BancoINVICOSdoFinalReport]
    ) -> RouteReturnSchema:
        try:
            ...
            validation_result = validate_and_extract_data_from_list(
                ...
                field_id="cta_cte",  # O el campo que identifique la fila en caso de error
            )
            ...
            return await sync_validated_to_repository(
                ...
                title="Sincronización SSCC Banco INVICO SDO FINAL",
                label="Banco INVICO SDO FINAL",
    ...
    # -------------------------------------------------
    async def export(self, params: BancoINVICOSdoFinalLiteFilter) -> StreamingResponse:
        # 1. Creamos el objeto de filtros normal
        search_params = BancoINVICOSdoFinalFullFilter(
        ...
        return self.export_to_excel(
            data_pairs=[(df, "Banco_INVICO_SDO_FINAL")],
            filename="reporte_banco_invico_sdo_final.xlsx",
        )
