...
class Rvicon03Service(
    BaseService[
        Rvicon03Report, Rvicon03Document, Rvicon03FullFilter, Rvicon03LiteFilter
    ]
):
    ...
    def __post_init__(self):
        ...
        super().__init__(
            ...
            filter_schema=Rvicon03FullFilter,  # <--- LE DECIMOS QUIÉN ES 'F'
        )

    # -------------------------------------------------
    async def add_many(self, data: List[Rvicon03Report]) -> RouteReturnSchema:
        try:
            # 1. Validar usando tu función genérica
            validation_result = validate_and_extract_data_from_list(
                data_list=data,
                model=Rvicon03Report,
                field_id="cta_contable",  # O el campo que identifique la fila en caso de error
            )
            ...
            return await sync_validated_to_repository(
                ...
                title="Sincronización SIIF RVICON03",
                label="RVICON03",
    ...
    # -------------------------------------------------
    async def export(self, params: Rvicon03LiteFilter) -> StreamingResponse:
        # 1. Creamos el objeto de filtros normal
        search_params = Rvicon03FullFilter(
        ...
        return self.export_to_excel(
            data_pairs=[(df, "SIIF_RVICON03")], filename="reporte_rvicon03.xlsx"
        )
